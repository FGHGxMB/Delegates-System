...
class ProtectedSettingsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ProtectedSettingsScreen> createState() =>
      _ProtectedSettingsScreenState();
}

class _ProtectedSettingsScreenState
    extends ConsumerState<ProtectedSettingsScreen> {

  final _cashSypCtrl = TextEditingController();
  final _cashUsdCtrl = TextEditingController();
  ...
  _cashSypCtrl.text = await settingsDao.getValue('cash_account_syp') ?? '';
  _cashUsdCtrl.text = await settingsDao.getValue('cash_account_usd') ?? '';
  ...
  Future<void> _saveChanges() async {
    final settingsDao = ref.read(settingsDaoProvider);
  await settingsDao.setValue('cash_account_syp', _cashSypCtrl.text.trim());
  await settingsDao.setValue('cash_account_usd', _cashUsdCtrl.text.trim());
  ...
  const SizedBox(height: 16),
  TextField(
  controller: _cashSypCtrl,
  decoration: const InputDecoration(
  labelText: 'رمز صندوق الليرة السورية (للمندوب)',
  hintText: 'مثال: 101',
  ...
  const SizedBox(height: 16),
  TextField(
  controller: _cashUsdCtrl,
  decoration: const InputDecoration(
  labelText: 'رمز صندوق الدولار (للمندوب)',
  hintText: 'مثال: 102',
  border: OutlineInputBorder(),
  ),
  ),
